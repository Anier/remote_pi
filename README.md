...
Оба процесса запускаются локально на одном компьютере. Сервер OpenCode доступен только через localhost (не暴露яется наружу), а Telegram Bot использует long polling для получения сообщений.
...
4. Параллельно обрабатывать SSE-события:
   ...
   - Событие "session.continue" → отметить завершение
...
**Обработка длинных ответов:** при превышении 4000 символов ответ разбивается на несколько сообщений.
...
**`formatErrorMessage(err)`** — форматирует ошибку для отправки в Telegram.

### `start-bot.ps1` — Скрипт запуска

PowerShell-скрипт, который:

1. Проверяет, запущен ли `opencode serve`
2. Если не запущен — запускает сервер в фоне
3. Переходит в директорию проекта
4. Запускает бота (`node src/index.js`)

**Флаг `-NoServe`:** пропускает запуск сервера (если он уже запущен отдельно):
